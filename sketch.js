...
function preload() {
  ...
  sound = loadSound("tropicana-soundroll-main-version-1725-02-15.mp3");  // preload the sound file
}
...
function createBox() {
  // prepare a box first
  strokeWeight(4);
  rect(0, 0, BOX_WIDTH + 200, BOX_HEIGHT);
  
  textSize(32);           // size of the text (pixels)
  fill(0, 90, 153);      // fill() takes R,G,B values as the color
  // draw the text in the box (x,y,width,height) with the color in fill()
  textAlign(LEFT);
  text('Hello Professor!', BOX_WIDTH/2, BOX_HEIGHT/2);   
